...
LIFO (англ. last in, first out, «последний пришёл — первый ушёл»)
// сначала выполняется defer fmt.Println(i), а затем i++
Отмена/повтор операций:
Стек можно использовать для реализации функций "отмена" и "повтор" в калькуляторе.
Каждая операция, выполненная пользователем, добавляется в стек. Нажатие кнопки "отмена"
извлекает последнюю операцию из стека и отменяет ее,
а "повтор" - повторяет последнюю операцию.
